...
опрашиваются с одной частотой:
  ...
  - "Крупные" (см. BIG_CITY_ICAO: Москва x3, СПб, Сочи, Новосибирск,
    Казань, Нижний Новгород) - прилёты каждый час днём.
  - Остальные (Екатеринбург/Челябинск/Омск/Самара/Краснодар) - прилёты раз
    в REGULAR_FETCH_INTERVAL_HOURS=3 часа днём.

ИЗМЕНЕНО 23.09.2026 (прямая просьба пользователя - "все аэропорты кроме
Москвы и Питера сделай обновление с 12 до 6.30 утра раз в 2 часа"):
пересмотрена логика ночного окна (см. MOSCOW_SPB_ICAO/should_fetch_arrivals
ниже) - Москва и СПб (см. MOSCOW_SPB_ICAO) теперь ЕДИНСТВЕННОЕ исключение,
которое опрашивается каждый час КРУГЛОСУТОЧНО, без прореживания вообще. ВСЕ
остальные аэропорты (и бывшие "крупные" - Сочи/Новосибирск/Казань/Нижний
Новгород, и обычные - Екатеринбург/Челябинск/Омск/Самара/Краснодар) ночью
(00:00 - ~06:30 по МЕСТНОМУ времени аэропорта, см. NIGHT_START_HOUR/
NIGHT_END_HOUR) опрашиваются раз в 2 часа - раньше "обычные" ночью не
опрашивались вообще (считалось, что реальных рейсов ночью там нет), теперь
по прямой просьбе пользователя опрашиваются, пусть и реже.
...
# ИЗМЕНЕНО 25.09.2026 (прямая просьба пользователя - "порог запаса сместим
# до 400 с резервом 100 до 500") - было int(500*0.7)=350, теперь порог
# задаётся напрямую фиксированным запасом (DAILY_SAFETY_RESERVE=100 до
# самой квоты), а не процентом - легче понять, сколько именно "воздуха"
# остаётся до реальной блокировки ключа.
DAILY_QUOTA = 500
DAILY_SAFETY_RESERVE = 100  # сколько запросов держим про запас до реальной квоты
DAILY_SAFETY_LIMIT = DAILY_QUOTA - DAILY_SAFETY_RESERVE  # = 400
...
# 90% - принятое допущение: все рейсы считаются загруженными на 90% от
# максимальной вместимости борта (обновлено 23.09.2026, было 85%).
AVERAGE_LOAD_FACTOR = 0.90

# Разбивка фактических пассажиров по классам обслуживания. Считается от
# уже применённой загрузки 90% (т.е. эти 90% пассажиров и есть 100% базы
# для разбивки эконом/бизнес) - обновлено 23.09.2026, было 85/15.
ECONOMY_SHARE = 0.90
BUSINESS_SHARE = 0.10
...
def estimate_passengers(thread):
    ...
    Вместимость борта x средняя загрузка (90%) x разбивка эконом/бизнес (90/10)."""
...
# прилёт и вылет каждый час". Ростов (URRP) отдельно просить не нужно - он
# уже помечен closed=true в config.json и полностью пропускается
# существующей проверкой airport.get('closed') в main() (см. ниже), как и
# раньше.
BIG_CITY_ICAO = {
...
# ИЗМЕНЕНО 23.09.2026 (прямая просьба пользователя - "все аэропорты кроме
# Москвы и Питера сделай обновление с 12 до 6.30 утра раз в 2 часа") -
# Москва+СПб теперь ЕДИНСТВЕННОЕ исключение без ночного прореживания вообще
# (опрашиваются каждый час круглосуточно), а ВСЕ остальные аэропорты (в т.ч.
# бывшие "крупные" Сочи/Новосибирск/Казань/Нижний Новгород, которые раньше
# тоже не прореживались ночью) теперь ночью опрашиваются раз в 2 часа - см.
# should_fetch_arrivals ниже.
MOSCOW_SPB_ICAO = {'UUEE', 'UUWW', 'UUDD', 'ULLI'}  # Шереметьево/Внуково/Домодедово/Пулково
REGULAR_FETCH_INTERVAL_HOURS = 3  # обычные (не "крупные") аэропорты - раз в 3 часа днём
NIGHT_START_HOUR = 0
# [0, 7) по МЕСТНОМУ времени аэропорта - аппроксимация "до 6:30", т.к.
# крон дёргает main() с точностью до часа (не до минуты, см. докстринг
# модуля выше) - час 6 (06:00-06:59) ещё считается ночным, час 7 уже нет.
NIGHT_END_HOUR = 7

def should_fetch_arrivals(icao, local_hour):
    """Решает, стоит ли опрашивать ПРИЛЁТЫ этого аэропорта в данный час его
    местного времени - см. комментарии у MOSCOW_SPB_ICAO/BIG_CITY_ICAO выше.
    Вызывается заново для каждого аэропорта при каждом запуске (крон - раз в
    час), поэтому сама разница в частоте получается без разных
    crontab-записей.

    Москва+СПб (MOSCOW_SPB_ICAO) - без прореживания, каждый час
    круглосуточно. Все остальные аэропорты ночью (NIGHT_START_HOUR..
    NIGHT_END_HOUR) опрашиваются раз в 2 часа - независимо от того,
    "крупный" это аэропорт или обычный. Днём разница по тирам сохраняется:
    "крупные" (BIG_CITY_ICAO) каждый час, обычные - раз в
    REGULAR_FETCH_INTERVAL_HOURS часов."""
    if icao in MOSCOW_SPB_ICAO:
        return True
    is_night = NIGHT_START_HOUR <= local_hour < NIGHT_END_HOUR
    if is_night:
        return local_hour % 2 == 0
