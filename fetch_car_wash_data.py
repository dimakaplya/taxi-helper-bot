#!/usr/bin/env python3
"""
Автомойки для кнопки "🚿 Мойки" (модуль "Инструменты водителя"). Источник -
OpenStreetMap через Overpass API (публичный, ключ не нужен). Вызывается
вручную/по расписанию (как fetch_parking_data.py, fetch_yandex_data.py и
fetch_timepad_data.py) - НЕ фоновой задачей внутри самого бота.
...
Теги shop=car_wash И amenity=car_wash (в OSM оба варианта встречаются).
Фильтр "бесплатно" не применяется - платные сервисы по определению.
...
OUTPUT_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'car_wash_data.json')
REQUEST_TIMEOUT = 100
...
def fetch_city_car_wash(bbox):
    south, west, north, east = bbox
    query = f'''[out:json][timeout:90];
(
  node["shop"="car_wash"]({south},{west},{north},{east});
  way["shop"="car_wash"]({south},{west},{north},{east});
...
def main():
    ...
    for bot_city, bbox in CITY_BBOX.items():
        logger.info(f"🔄 Тяну автомойки для {bot_city}...")
        try:
            points = fetch_city_car_wash(bbox)
            result['cities'][bot_city] = points
            logger.info(f"✅ {bot_city}: {len(points)} автомоек (после дедупа)")
        except Exception as e:
            logger.error(f"❌ Не удалось получить автомойки для {bot_city}: {e}")
