...
AIRPORTS_INFO = {
    'moscow': [
        # SVO (Шереметьево) - ТРИ отдельные записи с ОДНИМ И ТЕМ ЖЕ icao, по
        # одной на терминал B, C и D (см. AIRPORT_TERMINAL_ZONES; терминалы
        # E/F намеренно не отдельными точками - см. комментарий там) - по
        # просьбе пользователя, чтобы в списке аэропортов сразу было видно
        # раздельный выбор, а не только внутри карточки. 'zone_key'
        ...
        # так что все три записи получают один и тот же статус - это
        # осознанно, не баг.
        {'name': 'SVO (Шереметьево Терминал B)', 'emoji': '✈️', 'icao': 'UUEE', 'iata': 'SVO', 'zone_key': 'b'},
        {'name': 'SVO (Шереметьево Терминал C)', 'emoji': '✈️', 'icao': 'UUEE', 'iata': 'SVO', 'zone_key': 'c'},
...
# Шереметьево (UUEE) - по просьбе пользователя разделён на ТРИ отдельные
# точки: терминал B, терминал C и терминал D - каждый отдельным пунктом в
# списке аэропортов (обновлено 19.09.2026, было 2 зоны "B/C вместе" и
# "D/E/F вместе", пользователь попросил развести ещё и B с C). Терминалы E
# и F НЕ включены ни в одну зону и не показываются отдельными пунктами -
# по словам пользователя, у Яндекс.Go парковка/очередь для водителей есть
# только на B, C и D, так что E/F не имеют смысла как отдельная точка
# (рейсы туда просто не попадают ни в одну зону - это осознанно, не баг).
...
# Координаты терминалов B и C близки (соединены наземными галереями, общий
# подъезд), но не идентичны - взяты отдельно по официальным адресам (2ГИС).
...
AIRPORT_TERMINAL_ZONES = {
    'UUEE': {
        'b': {'coords': (55.981271, 37.414241), 'label': 'Терминал B'},
        'c': {'coords': (55.980579, 37.409522), 'label': 'Терминал C'},
...
# делать отдельными точками (нет там очереди Яндекс.Go) - рейсы в эти
...
def format_airport_queue_push(kind, airport, dist_km, zone_label=None):
    ...
    к какому именно терминалу ближе водитель ("Терминал B", "Терминал C"
...
async def show_airport_details(callback_query: types.CallbackQuery):
    ...
    for hour_offset in range(8):
        ...
        # airport - переменная выше, ДО цикла) - у Шереметьево все три
        # пункта в списке аэропортов (B, C и D) уже свои конкретные зоны,
