...
STATION_CITY = {
    ...
    # ДОБАВЛЕНО 19.09.2026 - раньше вокзалы покрывали только 6 из 12 городов
    # бота, теперь все 12 (см. STATIONS в fetch_trains_data.py).
    's9610189': 'novosibirsk',  # Новосибирск-Главный
    's9607404': 'ekb',          # Екатеринбург-Пасс.
    's9609235': 'chelyabinsk',  # Челябинск-Главный
    's9610384': 'omsk',         # Омск-Пасс.
    's9606096': 'samara',       # Самара
    's9612913': 'rostov',       # Ростов-Главный
}
...
STATION_CAPACITY = {
    ...
    # ДОБАВЛЕНО 19.09.2026 - тем же методом ГРУБОЙ прикидки по размеру
    # города/значимости узла относительно откалиброванных московских цифр
    # (станционных данных по этим вокзалам в открытых источниках нет).
    's9610189': 1000,  # Новосибирск-Главный - крупнейший узел Транссиба, ~1,6 млн населения
    's9607404': 1000,  # Екатеринбург-Пасс. - крупный узел Урала, ~1,5 млн населения
    's9609235': 800,   # Челябинск-Главный - ~1,2 млн населения
    's9610384': 750,   # Омск-Пасс. - на Транссибе, ~1,1 млн населения
    's9606096': 900,   # Самара - узел Куйбышевской ж/д, ~1,1 млн населения
    's9612913': 900,   # Ростов-Главный - южные ворота (Кавказ/Крым/Сочи), ~1,1 млн населения
}
...
AIRPORTS_INFO = {
    'moscow': [
        # SVO (Шереметьево) - ДВЕ отдельные записи с ОДНИМ И ТЕМ ЖЕ icao: B/C
        # вместе (общая точка, координаты - среднее между терминалами B и C,
        # см. AIRPORT_TERMINAL_ZONES) и отдельно D. Терминалы E/F намеренно не
        # входят ни в одну зону - см. комментарий там. 'zone_key' используется
        # в show_airport_details/compute_current_hour_load/
        ...
        # не баг.
        {'name': 'SVO (Шереметьево B/C)', 'emoji': '✈️', 'icao': 'UUEE', 'iata': 'SVO', 'zone_key': 'bc'},
