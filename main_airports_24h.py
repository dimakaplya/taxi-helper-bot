...
# Файл с реальными данными, который генерирует fetch_yandex_data.py (запускается локально)
        ...
        if data and airport_icao in data.get('airports', {}):
            ...
            for flight_data in raw_flights:
                ...
                flights.append({
                    ...
                    'passengers': flight_data.get('passengers', 170)
                })
...
async def show_airport_info(callback_query: types.CallbackQuery):
    ...
    for i, airport in enumerate(airports):
        ...
        if flights:
            ...
            avg_load = (total_passengers / len(flights) / AIRPORT_CAPACITY.get(airport['icao'], 1000)) * 100
        else:
            avg_load = 0
...
async def show_airport_availability(callback_query: types.CallbackQuery):
    ...
    for airport in airports:
        ...
        capacity = AIRPORT_CAPACITY.get(airport['icao'], 1000)
        total_passengers = sum(f.get('passengers', 0) for f in arrivals + departures)
        current_load = (total_passengers / capacity) * 100 if total_passengers > 0 else 0
        if current_load < 50:
            status = "✅ Свободен"
            load_emoji = "🟢"
