        ...
        if isinstance(method, SendMessage):
            ...
            try:
                if has_reply_keyboard or skip_trim:
                    # ОТКАЧЕНО 22.09.2026 (реальный баг, пойман пользователем
                    # по скриншоту - "А где выбор меню"): убирали это
                    # исключение в предыдущей правке по прямой просьбе
                    # ("чистить вообще всё"), но на практике это удалило
                    # ПОСЛЕДНЕЕ сообщение с нижним меню раньше, чем пришло
                    # новое с тем же меню (например, между "Выбери город" и
                    # следующим сообщением без клавиатуры типа VPN-приглашения
                    # проходит чистка) - меню у пользователя реально пропало.
                    # Возвращено как было: сообщения с ReplyKeyboardMarkup (и
                    # карточки событий через _skip_message_trim) не участвуют
                    # в общей чистке, чтобы нижнее меню никогда не исчезало.
                    pass
                else:
