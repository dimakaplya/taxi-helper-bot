...
# ИЗМЕНЕНО 21.09.2026 (по просьбе пользователя разводить время жизни МЕТОК
# НА КАРТЕ по типу события - перекрытия висят на карте 24ч, обычные ДТП
# только 3ч, см. MAP_CLOSURE_TTL_HOURS/MAP_INCIDENT_TTL_HOURS и их
# применение в handle_map_road_events_api в main.py): LOOKBACK_HOURS здесь
# по-прежнему определяет, что вообще СОБИРАЕТСЯ и попадает в обычный список
# "⛔ Дорожные события" внутри бота (тот список НЕ разводится по типу -
# см. отдельное решение пользователя, там остаётся общий срок) - поднят с
# 6 до 24ч, чтобы данных хватало для более долгого TTL перекрытий на карте;
# фактическое ограничение "сколько висит НА КАРТЕ" применяется отдельно в
# main.py по полю 'time' каждого события и его 'is_closure'.
LOOKBACK_HOURS = 24
MAX_MESSAGES_PER_CITY = 30  # сколько последних сообщений хранить/показывать на город (было 15 - увеличено вслед за LOOKBACK_HOURS 6ч->24ч, чтобы не срезать события в конце окна)
