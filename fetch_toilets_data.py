#!/usr/bin/env python3
"""
Общественные туалеты для кнопки "🚻 Туалеты рядом" (модуль "Инструменты
...
В отличие от парковок, для туалетов НЕ применяется фильтр "бесплатно" -
amenity=toilets в OSM почти всегда общедоступные точки (внутри ТЦ, на
заправках, в парках), доп. фильтрация по fee/access для них не так надёжна,
как для парковок (специфика тегов другая), поэтому берём все точки как есть.
...
def main():
    ...
    for bot_city, bbox in CITY_BBOX.items():
        logger.info(f"🔄 Тяну туалеты для {bot_city}...")
        try:
            points = fetch_city_toilets(bbox)
            result['cities'][bot_city] = points
            logger.info(f"✅ {bot_city}: {len(points)} туалетов (после дедупа)")
        except Exception as e:
            logger.error(f"❌ Не удалось получить туалеты для {bot_city}: {e}")
