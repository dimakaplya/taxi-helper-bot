#!/usr/bin/env python3
"""
Алкомаркеты для кнопки "🍷 Алкомаркеты" (модуль "Инструменты водителя").
...
OUTPUT_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'alcohol_data.json')
REQUEST_TIMEOUT = 100
...
(
  node["shop"="alcohol"]({south},{west},{north},{east});
  way["shop"="alcohol"]({south},{west},{north},{east});
);
out center tags;'''
