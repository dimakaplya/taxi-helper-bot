...
# Бот-город -> bbox (south, west, north, east). Пока только Москва (МКАД +
# запас) - остальные города можно дописывать сюда же по мере надобности,
# формат parking_data.json (cities: {бот-город: [...]})  это уже поддерживает.
CITY_BBOX = {
