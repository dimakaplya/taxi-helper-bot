...
=== АВТОМАТИЧЕСКИЙ ЕЖЕМЕСЯЧНЫЙ ЗАПУСК (ДОБАВЛЕНО 24.09.2026) ===
По прямой просьбе пользователя ("раз в месяц собирать... настроить это
чтоб повторно было без моего участия") - вместо ручного запуска или
crontab, см. run_monthly_afisha_update.sh + com.taxihelper.timepad-monthly.plist
(launchd, а не cron - надёжнее на macOS: сам досрочно выполнит пропущенный
запуск после сна/перезагрузки компьютера, чего голый cron не умеет).
Обёртка сама делает: запуск этого скрипта -> git add/commit -> git push
(полностью автоматически, БЕЗ участия пользователя, только если файл
реально изменился) -> Telegram-уведомление админу (успех/ошибка). Разовая
настройка - см. AFISHA_MONTHLY_SETUP.md рядом с этим файлом.
...
=== ПОЧЕМУ ТЕПЕРЬ ВСЕ 12 ГОРОДОВ БОТА ===
ДОБАВЛЕНО 23.09.2026 (прямая просьба пользователя "другие города"), ИЗМЕНЕНО
24.09.2026: живой пробный запрос к /v1/events.json по каждому из 12 городов
бота (с тем же фильтром категорий/tickets_total) тогда показал, что у
Челябинска, Омска, Самары и Ростова-на-Дону на TimePad почти нет крупных
событий нужных категорий (0 штук на первой странице выдачи), поэтому их
временно не включали, чтобы не тратить лимит запросов (60/мин с одного
IP) на города, где физически нечего показывать. Теперь, когда сбор стал
ЕЖЕМЕСЯЧНЫМ автоматическим фоновым процессом (а не ручным разовым
действием), эта экономия уже не имеет смысла - лишние несколько секунд раз
в месяц ничего не стоят, а крупные события в этих городах могут появляться
со временем. Все 4 города возвращены в TIMEPAD_CITY_MAP - для них просто
может честно приходить 0 событий в какие-то месяцы, это ожидаемо и не
ошибка.
"""
import os
import time
...
# Бот-город -> название города у TimePad (параметр cities). См. "ПОЧЕМУ
# ТЕПЕРЬ ВСЕ 12 ГОРОДОВ БОТА" в шапке файла - chelyabinsk/omsk/samara/rostov
# вернули обратно 24.09.2026, когда сбор стал ежемесячным автоматическим.
TIMEPAD_CITY_MAP = {
    'moscow': 'Москва',
    'spb': 'Санкт-Петербург',
